(function(){angular.module('fuzzy').controller("fuzzyController",fuzzyController).controller("ModalDemoCtrl",ModalDemoCtrl).controller("ModalInstanceCtrl",ModalInstanceCtrl);fuzzyController.$inject=['countryFactory','territoriesFactory','Upload','$http','$timeout','$q'];ModalDemoCtrl.$inject=['$modal','$log'];ModalInstanceCtrl.$inject=['$modalInstance'];function ModalDemoCtrl($modal,$log){var vm=this;vm.animationsEnabled=true;vm.open=open;vm.toggleAnimation=toggleAnimation;function open(size){var modalInstance=$modal.open({animation:vm.animationsEnabled,templateUrl:'myModalContent.html',controller:'ModalInstanceCtrl',size:size,resolve:{items:function(){}}});modalInstance.result.then(function(selectedItem){},function(){$log.info('Modal dismissed at: '+new Date());});};function toggleAnimation(){vm.animationsEnabled=!vm.animationsEnabled;};};function ModalInstanceCtrl($modalInstance){var vm=this;vm.ok=ok;vm.cancel=cancel;function ok(){$modalInstance.close();};function cancel(){$modalInstance.dismiss('cancel');};};function fuzzyController(countryFactory,territoriesFactory,Upload,$http,$timeout,$q){var vm=this;vm.btn_fuzzy_disabled=true;vm.btn_cancel_disabled=true;vm.btn_sendcrm_disabled=true;vm.btn_label="Run Fuzzy!"
...
function fillTerritory(idx,selectedCountry){console.log('idx ::::'+idx)
vm['selectedCountry_'+idx]=selectedCountry
console.log('selectedCountry ::::'+selectedCountry)
vm['countryMapping'+idx]={}
angular.copy(vm.countryMapping,vm['countryMapping'+idx]);console.log('countryMapping ::::'+JSON.stringify(vm.countryMapping))
...
getResults(data.job_id,data.i)}).error(function(data,status,headers,config){console.log('error status: '+status);vm.btn_fuzzy_disabled=false;vm.job_ids=remove(vm.job_ids,data.job_id)})}
...
console.log('The current status is : '+data)}else if(status===200){ vm.job_ids=remove(vm.job_ids,job_id)
...
if(vm.job_ids.length==0){vm.btn_label="Run Fuzzy!"}
return false;}
 
timeout=$timeout(tracker,2000);}).error(function(error){});};tracker();}
function remove(arr,element){var i=arr.indexOf(element);if(i!=-1){arr.splice(i,1);}
return arr}
